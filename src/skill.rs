...
fn is_installed(base: &Path, name: &str) -> bool {
...
pub fn list() -> Result<(), String> {
    ...
    for skill in SKILLS {
        let proj = project_root
            .as_ref()
            .map_or("-".to_string(), |r| if is_installed(r, skill.name) { "\u{2713}".into() } else { "-".into() });
...
fn install_one(base: &Path, skill: &BundledSkill) -> Result<bool, String> {
...
pub fn install(name: Option<&str>, project: bool, global: bool) -> Result<(), String> {
    ...
    match name {
        Some(n) => {
            let skill = find_skill(n)
                .ok_or_else(|| format!("unknown skill: {n}"))?;
            if install_one(&base, skill)? {
...
fn remove_skill_dir(base: &Path, name: &str) -> Result<bool, String> {
    let dir = skill_dir(base, name);
    if !dir.exists() {
        return Ok(false);
    }
...
pub fn uninstall(name: Option<&str>, project: bool, global: bool) -> Result<(), String> {
    ...
    match name {
        Some(n) => {
            // validate the name is a known skill
            find_skill(n).ok_or_else(|| format!("unknown skill: {n}"))?;
            if remove_skill_dir(&base, n)? {
        ...
        None => {
            for skill in SKILLS {
                if remove_skill_dir(&base, skill.name)? {
