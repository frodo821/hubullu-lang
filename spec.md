# Hubullu — Artificial Natural Language Dictionary Format Specification

## 概要

Hubulluは人工自然言語の辞書を記述するためのドメイン固有言語。
設計原則は以下の通り：
...
```
ソースファイル群 (.hu)
  profile.hu          # タグ軸・活用クラス・形態クラス・音韻規則定義
  entries/**.hu       # エントリ定義

          ↓ コンパイラ

dictionary.huc         # 生成物（git管理不要）
  entries              # エントリ本体
  entry_tags           # タグ（axis=value）
  entry_meanings       # 多義エントリの個別意味
  headword_scripts     # 複数書記体系のheadword
  forms                # 活用形 → エントリIDの逆引き
  links                # エントリ間リンクグラフ
  tagaxis_meta         # タグ軸のメタ情報（表示名）
  inflection_meta      # 活用クラスのメタ情報
  inflection_display   # 活用クラスの表示名
  inflection_axes      # 活用クラスの展開軸
  render_config        # レンダリング設定
  entries_fts          # FTS5全文検索
```
...
- 取り込み対象は宣言型（`tagaxis`, `@extend`, `inflection`, `phonrule`）のみ
...
- `@export` も同様にトップレベル項目として使用可能（後述）

### 標準ライブラリ

`std:` スキームで組み込みモジュールをインポートできる。

```
@use * from "std:ipa"      # 標準ライブラリの ipa モジュール
```

標準ライブラリモジュールはコンパイラに埋め込まれており、ファイルシステム上のパス解決は行われない。

### `@export` — 再エクスポート

インポートしたシンボルまたは別ファイルのシンボルを再エクスポートする。

```ebnf
export_decl = "@export" ("use" | "reference") import_target ("from" string_literal)? ;
```

**形式1: インポート済みシンボルの再エクスポート**

```
@use * from "core.hu"
@export use *             # coreの全宣言を再エクスポート
@export use tense         # tenseのみ再エクスポート
```

**形式2: インポート＋再エクスポート（ワンステップ）**

```
@export use * from "core.hu"          # core.huをインポートしつつ再エクスポート
@export reference * from "entries.hu"  # entries.huのエントリをインポート＋再エクスポート
```

`@export` は `@use`/`@reference` と同じ構文で `import_target` を共有する。
`@export use` は宣言型（tagaxis, @extend, inflection, phonrule）を、`@export reference` はentryを再エクスポートする。
...
音韻変化（母音調和・子音同化など）を宣言的に記述する。
`inflection` のルール右辺や `compose` チェーンから適用される。

```ebnf
phonrule_decl = "phonrule" ident "{" (class_def | map_def | rewrite_rule)* "}" ;
```
...
class_body = "[" string_literal ("," string_literal)* "]"   (* リテラルリスト *)
           | ident ("|" ident)*                              (* 他クラスの和集合 *) ;
```

リテラルリストは各文字を文字列として列挙する。和集合は既存クラス名を `|` で結合する。

### マップ（`map`）

```ebnf
map_def    = "map" ident "=" ident "->" "match" "{" map_arm ("," map_arm)* ("," else_arm)? "}" ;
map_arm    = string_literal "->" map_result ;
else_arm   = "else" "->" map_result ;
map_result = string_literal | ident (* パラメータ変数 *) ;
```

入力文字をパターンマッチで変換する。`else` 節は省略可能。

### 書き換え規則

```ebnf
rewrite_rule = pattern "->" replacement ("/" context)? ;
pattern      = ident       (* クラス名 *)
             | string_literal (* リテラル *) ;
replacement  = ident       (* マップ名 *)
             | string_literal (* リテラル *)
             | "null"      (* 削除 *) ;
context      = context_elem* "_" context_elem* ;
context_elem = ident       (* クラス *)
             | "!" ident   (* クラスの否定 *)
             | "+"         (* 形態素境界 *)
             | "^"         (* 語頭 *)
             | "$"         (* 語末 *)
             | "(" context_elem ("|" context_elem)* ")"  (* 選択 *)
             | string_literal (* リテラル *)
             | context_elem "*" (* 0回以上の繰り返し *) ;
```

- `_` は書き換え対象の位置を示す
- `+` は形態素境界（`compose` 時のスロット間接合部）にマッチ（語頭・語末にもマッチ）
- `^` は語頭（文字列先頭）にのみマッチ
- `$` は語末（文字列末尾）にのみマッチ
- `(a | b)` はいずれかの要素にマッチ（選択）
- `!class` はそのクラスに属さない文字にマッチ
- `*` は直前の要素の0回以上の繰り返し
- 書き換えは収束するまで反復適用される

### `phonrule` の適用

`phonrule` は2つの文脈から適用できる：

**1. ルール右辺での適用：**

```
[tense=present, number=pl, _] -> harmony(`{root}ler`)
```

**2. compose チェーンでの適用：**

```
compose harmony(root + tense_sfx + number_sfx)
```

`compose` チェーンで適用される場合、スロット間の接合部に形態素境界マーカーが挿入され、
`+` コンテキスト要素によるマッチが可能になる。
...
inflection には省略可能な `display` フィールドを付与できる。
UI/辞書アプリでパラダイム名を表示する際に使用する。

```
inflection strong_I display { ja: "強変化I類", en: "Strong Class I" } for {tense, person, number} {
  ...
}
```
...
### `apply` — パラダイム全体への音韻規則適用

inflection body の先頭に `apply` 式を記述することで、全ルール結果（委譲以外）に音韻規則を一括適用できる。
...
```ebnf
apply_expr = "apply" (ident "(" apply_expr ")" | "cell") ;
```

`cell` はルール評価結果を表す終端で、`harmony(cell)` のようにネスト可能。
委譲ルール（`->` の右辺が別のinflection）の結果には適用されない。
...
`compose` チェーンに `phonrule` を適用することで、スロット合成後の音韻変化を宣言的に記述できる：

```
inflection agglutinative_verb for {tense, number} {
  requires stems: root
...
compose_term = ident "(" compose_expr ")"    (* phonrule適用 *)
             | ident                          (* スロット参照 *) ;
```
...
ルールの右辺にも `phonrule` を適用できる。テンプレート結果に対して音韻変化を実行する：
...
rule_rhs = template_literal
         | "null"
         | ident "(" rule_rhs ")"    (* phonrule適用 — 再帰的にネスト可能 *)
         | ident "[" delegate_tag_list "]" ... ;
```
...
`[form_spec]` が複数のセルに一致しても、それらが同一の値を返す場合は許容され、その値が返される（`_` ワイルドカード等で1つのルールが複数セルに展開されたケースに対応するため）。複数のセルが**異なる値**を返す場合のみエラー（一意解決を強制）。
...
token:
  ...
  <tag>...</tag>   # XMLライクなタグ（属性付き対応、レンダリング時に出力）
  <br/>            # 自己閉じタグ
```
...
- `<tag attr="val">...</tag>` — XMLライクなタグでトークンをラップ。子要素としてトークン列を含む
- `<tag/>` — 自己閉じタグ。子要素なし
...
`.hut` ファイルのトークン結合方法を制御する。`.hu` ファイル内に記述し、SQLiteに保存される。
...
| フィールド            | 意味                                     | デフォルト   |
| --------------------- | ---------------------------------------- | ------------ |
| `separator`           | トークン間に挿入する文字列               | `" "`        |
| `no_separator_before` | この文字で始まるトークンの前はセパレータ省略 | `".,;:!?"` |

---

## `.hut` ファイル — トークンリストレンダリング

`.hut`（hubullu template）ファイルは、コンパイル済み辞書のエントリを参照してテキストをレンダリングする。
`@reference` ディレクティブで辞書ソースを指定し、その後にトークン列を記述する。

```
@reference * from "lang.hu"
@reference * as lat from "latin.hu"

ic[] faren[tense=present, person=1, number=sg] "."
lat.aqua[]
"mal" ~ "bon" ~ "a" hundo[]
```
...
フェーズ1：収集
  ...
  2. @reference のパスを収集してエントリファイルを列挙
     → 既出ファイルはスキップ（循環を許容）
  3. 全ファイルをパースしてASTを生成
  4. 宣言型（tagaxis, @extend, inflection, phonrule）をホイスト
  5. 全 entry の ID をシンボルテーブルに登録（中身は未解決）

フェーズ2：解決
  全シンボルが揃った状態で以下を検証・解決
  - @extend の適用（値追加・競合チェック）
  - phonrule の検証
  - inflection の検証（軸存在、ルール検証）
...
| テーブル              | カラム                                           | 内容                                                                                         |
| --------------------- | ------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| `entries`             | id, name, headword, meaning, inflection_class_id, etymology_proto, etymology_note | エントリ本体（inflection_class_idはinflection_metaへの参照、etymology_proto/etymology_noteは語源情報） |
...
| `inflection_meta`     | id, name                                         | 活用クラスのメタ情報                                                                        |
| `inflection_display`  | inflection_id, display_lang, display_text        | 活用クラスの多言語表示名                                                                    |
| `inflection_axes`     | inflection_id, axis_name                         | 活用クラスの展開軸                                                                          |
| `render_config`       | key, value                                       | レンダリング設定（separator, no_separator_before）                                          |
| `compile_meta`        | key, value                                       | コンパイルメタ情報（エントリポイントのディレクトリなど）                                    |
| `name_resolution`     | file_hash, name, entry_id                        | ファイルハッシュごとのシンボルスコープ（.hut参照解決用）                                    |
| `entries_fts`         | name, headword, meaning                          | FTS5全文検索仮想テーブル（content='entries'）                                               |
...
| `idx_links_dst`            | links(dst_entry_id)          |
| `idx_entry_tags`           | entry_tags(entry_id)         |
| `idx_entry_tags_axis`      | entry_tags(axis, value)      |
| `idx_inflection_display`   | inflection_display(inflection_id) |
| `idx_inflection_axes`      | inflection_axes(inflection_id)    |
| `idx_name_resolution_hash` | name_resolution(file_hash)        |
...
```
hubullu compile <INPUT> [--output <OUTPUT.huc>]
hubullu lint <INPUT> [--fix]
hubullu render <INPUT.hut> [--huc <PATH>]
hubullu render --dir <DIR> -o <OUTDIR> [--huc <PATH>] [--title <TITLE>]
hubullu lsp                          # Language Server Protocol サーバー起動（lsp feature有効時）
hubullu skill list|show|install|uninstall  # Claude Code skill管理
```
